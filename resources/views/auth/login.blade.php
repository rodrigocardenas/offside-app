...
<x-dynamic-layout :layout="$layout">
            ...
            <div style="background: {{ $bgCard }}; border-radius: 12px; padding: 20px; border: 1px solid {{ $borderColor }}; margin-bottom: 24px;">
                ...
                <form method="POST" action="{{ route('login') }}" style="display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <button 
                        ...
                        style="width: 100%; padding: 12px 16px; background: #fff; color: #333; border: 1px solid {{ $borderColor }}; border-radius: 10px; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none;">
    ...
    <script>
        ...
        /**
         * Manejador de Google Login para Capacitor
         * Obtiene la URL de OAuth de Google y la abre en el navegador
         */
        async function handleGoogleLogin() {
            ...
            try {
                // Deshabilitar botón
                btn.disabled = true;
                btn.innerHTML = '<span style="display: flex; align-items: center; gap: 8px;"><i class="fas fa-spinner fa-spin"></i> Cargando...</span>';

                // Usar el servicio de Google OAuth (será importado por Vite)
                if (window.googleOAuthService) {
                    await window.googleOAuthService.startGoogleLogin();
                } else {
                    // Fallback: abrir URL directamente
                    const response = await fetch('{{ route("api.mobile.google.url") }}', {
                        method: 'GET',
                        headers: {
                    ...
                    console.log('Abriendo URL de Google OAuth:', data.url);
                    window.open(data.url, '_blank');
                }

            } catch (error) {
                console.error('Google login error:', error);
                alert('Error al intentar login con Google: ' + error.message);
            } finally {
                // Restaurar botón
                btn.disabled = false;
