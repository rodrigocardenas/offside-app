...
<div class="min-h-screen bg-slate-950 py-12 text-white">
    <div class="mx-auto flex max-w-3xl flex-col gap-10 px-6">
        ...
        <header>
            ...
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.35em] text-sky-300">Nueva Pregunta</p>
                <h1 class="mt-3 text-4xl font-semibold">Crear Pregunta</h1>
                <p class="mt-2 text-base text-slate-400">
                    Configura una nueva pregunta con sus opciones, puntuación y disponibilidad.
                </p>
            </div>
        </header>
        ...
        <form action="{{ route('admin.questions.store') }}" method="POST" class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/30"
                          placeholder="Detalles adicionales sobre la pregunta...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                    ...
                    <select id="category" name="category"
                        ...
                        <option value="predictive" {{ old('category') === 'predictive' ? 'selected' : '' }}>Predictiva</option>
