...
<div id="feedbackModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.6); display: none; align-items: center; justify-content: center; z-index: 9999; padding: 16px;">
    <div style="background: {{ $bgSecondary ?? '#2a2a2a' }}; border: 1px solid {{ $borderColor ?? '#333' }}; border-radius: 16px; width: 100%; max-width: 500px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.4);">
        ...
        <div style="padding: 20px 24px; border-bottom: 1px solid {{ $borderColor ?? '#333' }}; display: flex; justify-content: space-between; align-items: center;">
            <h3 style="margin: 0; font-size: 18px; font-weight: 700; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.title') }}</h3>
        ...
        <form id="feedbackForm" action="{{ route('feedback.store') }}" method="POST" style="display: flex; flex-direction: column; padding: 24px;">
            ...
            <div style="margin-bottom: 20px;">
                <label for="type" style="display: block; font-size: 14px; font-weight: 600; margin-bottom: 8px; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.type_label') }}</label>
                <select id="type" name="type" required
                    ...
                    <option value="suggestion" style="background: {{ $bgTertiary ?? '#333333' }}; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.suggestion') }}</option>
                    <option value="bug" style="background: {{ $bgTertiary ?? '#333333' }}; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.bug') }}</option>
                    <option value="compliment" style="background: {{ $bgTertiary ?? '#333333' }}; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.compliment') }}</option>
                    <option value="other" style="background: {{ $bgTertiary ?? '#333333' }}; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.other') }}</option>
                </select>
            </div>

            <!-- Mensaje -->
            <div style="margin-bottom: 20px;">
                <label for="message" style="display: block; font-size: 14px; font-weight: 600; margin-bottom: 8px; color: {{ $textPrimary ?? '#ffffff' }};">{{ __('views.feedback.message_label') }}</label>
            ...
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 24px;">
                ...
                <label for="is_anonymous" style="margin: 0; font-size: 14px; color: {{ $textSecondary ?? '#b0b0b0' }}; cursor: pointer;">{{ __('views.feedback.anonymous') }}</label>
            </div>
